...
class Guitar():
    ...
    FENDER_STYLE = {'scale':647.7,#mm, 25.5 inches, per StewMac specs
                    'nut_width':43.48,#mm, per StewMac specs
                    'nut_string_spread':35.56,#mm, per StewMac specs
                    'bridge_string_spread':52.37,#mm
                    'n_fret':22,
                    'tuners':(6,0),
                    'nut_depth':3.18}
    ...
    def __init__(self, **kwargs):
        if 'scale' in kwargs:
            self.long_scale = kwargs['scale']
            self.short.scale = kwargs['scale']
        else:
            self.long_scale = kwargs['long_scale'] if 'long_scale' in kwargs else 647.7#mm
            self.short_scale = kwargs['short_scale'] if 'short_scale' in kwargs else 647.7#mm
        #self.neutral_fret = kwargs['neutral_fret'] if 'neutral_fret' in kwargs else 0 if self.long_scale == self.short_scale else 10
        self.nut_width = kwargs['nut_width'] if 'nut_width' in kwargs else 44.07#mm
        self.nut_string_spread = kwargs['nut_string_spread'] if 'nut_string_spread' in kwargs else 35.05#mm
        ...
        self.tuners = kwargs['tuners'] if 'tuners' in kwargs else (0,6)
        self.n_string = self.tuners[0] + self.tuners[1]
        self.scale = kwargs['scale'] if 'scale' in kwargs else 647.7#[self.long_scale + i*(self.short_scale-self.long_scale)/(self.n_string-1) for i in range(self.n_string)]
        self.delta = (self.nut_width - self.nut_string_spread)/2
        self.nut_depth = kwargs['nut_depth'] if 'nut_depth' in kwargs else 4.8#mm
        self.binding_thickness = kwargs['binding_thickness'] if 'binding_thickness' in kwargs else 0#mm
        self.southpaw = kwargs['southpaw'] if 'southpaw' in kwargs else False
        if self.tuners[0]*self.tuners[1] == 0:
            self.tuner_spacing = 25.4*15/16
            self.tuner_incline = math.degrees(math.asin(self.nut_string_spread/(5*self.tuner_spacing)))
            self.fb_nut_overhang = 10#mm
            first_tuner = 45
            
        else:
            self.tuner_spacing = 25.4*(1+3/8)
            self.tuner_incline = math.degrees(math.asin(self.nut_string_spread/(5*self.tuner_spacing)))
            self.fb_nut_overhang = 0
            first_tuner = 50
        ...
        self.l_tuners = [first_tuner+i*tuner_x_delta for i in range(self.tuners[0])]
        self.r_tuners = [first_tuner+i*tuner_x_delta for i in range(self.tuners[1])]
        self.svg_name = kwargs['svg_name'] if 'svg_name' in kwargs else 'guitar.svg'
        self.x0 = 1000
        self.y0 = 225
        self.root = self.draw()
    ...
    def draw(self, docwidth=1300, docheight=450, docunit='mm'):
        ...
        namedview = etree.SubElement(root, 
                                     ...
                                     **{'id':'base',
                                        ...
                                        '{%s}current-layer'%NSMAP['inkscape']: 'fretboard',
                                        'showgrid':'false',
                                        'showguide':'false',
        ...
        nut_end_half_width = self.fretboard_width_at(-self.nut_depth)/2
        fb_overhang_half_width = self.fretboard_width_at(-(self.nut_depth + self.fb_nut_overhang))/2
        
        board_path_coords =[(x0, y0+fretboard_start_half_width),
                            (x0-fretboard_length, y0+fretboard_end_half_width),
                            (x0-fretboard_length, y0-fretboard_end_half_width),
                            (x0, y0-fretboard_start_half_width)]
        board_nut_coords = [(x0, y0-fretboard_start_half_width),
                            (x0, y0+fretboard_start_half_width)]
        nut_depth_coords = [(x0+self.nut_depth, y0-nut_end_half_width),
                            (x0+self.nut_depth, y0+nut_end_half_width)]
        nut_top_coords = [(x0, y0-fretboard_start_half_width),
                          (x0+self.nut_depth, y0-nut_end_half_width)]
        nut_bot_coords = [(x0, y0+fretboard_start_half_width),
                          (x0+self.nut_depth, y0+nut_end_half_width)]
        fb_overhang_coords = [(x0+self.nut_depth+self.fb_nut_overhang, y0-fb_overhang_half_width),
                              (x0+self.nut_depth+self.fb_nut_overhang, y0+fb_overhang_half_width)]
        overhang_top_coords = [(x0+self.nut_depth, y0-nut_end_half_width),
                               (x0+self.nut_depth+self.fb_nut_overhang, y0-fb_overhang_half_width)]
        overhang_bot_coords = [(x0+self.nut_depth, y0+nut_end_half_width),
                               (x0+self.nut_depth+self.fb_nut_overhang, y0+fb_overhang_half_width)]
        
        bx0 = x0 + self.nut_depth + self.fb_nut_overhang if self.fb_nut_overhang else x0
        bx1 = x0 - fretboard_length + self.binding_thickness
        slope = (fretboard_end_half_width - fretboard_start_half_width)/fretboard_length
        bytop = lambda x: y0 - (-slope*(x-x0) + fretboard_start_half_width - self.binding_thickness*math.sqrt(1+slope**2))
        bybot = lambda x: y0 - (bytop(x)-y0)
        by0top = bytop(bx0)
        by0bot = bybot(bx0)
        by1top = bytop(bx1)
        by1bot = bybot(bx1)
        binding_path_coords = [(bx0, by0top),
                               (bx1, by1top),
                               (bx1, by1bot),
                               (bx0, by0bot)]
        
        
        neck_layer = etree.SubElement(root,
                                      'g',
                                      **{'{%s}groupmode'%NSMAP['inkscape']:'layer',
                                         'id':'neck',
                                         '{%s}label'%NSMAP['inkscape']:'Neck',
                                         'style': 'display:inline'})
        
        fb_layer = etree.SubElement(root,
                             'g',
                             **{'{%s}groupmode'%NSMAP['inkscape']:'layer',
                                'id':'fretboard',
                                '{%s}label'%NSMAP['inkscape']:'Fretboard',
                                'style': 'display:inline'})
        f_layer = etree.SubElement(root,
                             'g',
                             **{'{%s}groupmode'%NSMAP['inkscape']:'layer',
                                'id':'frets',
                                '{%s}label'%NSMAP['inkscape']:'Frets',
                                'style': 'display:inline'})
        fm_layer = etree.SubElement(root,
                             'g',
                             **{'{%s}groupmode'%NSMAP['inkscape']:'layer',
                                'id':'fretmarkers',
                                '{%s}label'%NSMAP['inkscape']:'Fret Markers',
                                'style': 'display:inline'})
        hs_layer = etree.SubElement(root,
                             'g',
                             **{'{%s}groupmode'%NSMAP['inkscape']:'layer',
                                'id':'headstock',
                                '{%s}label'%NSMAP['inkscape']:'Headstock',
                                'style': 'display:inline'})
        pup_layer = etree.SubElement(root,
                             'g',
                             **{'{%s}groupmode'%NSMAP['inkscape']:'layer',
                                'id':'pickups',
                                '{%s}label'%NSMAP['inkscape']:'Pickups',
                                'style': 'display:inline'})
        str_layer = etree.SubElement(root,
                             'g',
                             **{'{%s}groupmode'%NSMAP['inkscape']:'layer',
                                'id':'strings',
                                '{%s}label'%NSMAP['inkscape']:'Strings',
                                'style': 'display:inline'})
        board_path = etree.SubElement(neck_layer,
                                      'path',
                                      **{'style':LINE_STYLE,
                                         'd':self.path_string(board_path_coords),
                                         'id':'boardpath'})
        binding_path = etree.SubElement(fb_layer,
                                        'path',
                                        **{'style': LINE_STYLE,
                                           'd': self.path_string(binding_path_coords),
                                           'id': 'bindingpath'})
        nut_path = etree.SubElement(neck_layer,
                                    'path',
                                    **{'style':LINE_STYLE,
                                       'd':self.path_string(board_nut_coords),
                                       'id':'nutpath'})
        
        if self.fb_nut_overhang:
            nut_top_path = etree.SubElement(neck_layer,
                                            'path',
                                            **{'style':LINE_STYLE,
                                               'd':self.path_string(nut_top_coords),
                                               'id':'nuttop'})
            nut_depth_path = etree.SubElement(neck_layer,
                                              'path',
                                              **{'style':LINE_STYLE,
                                                 'd':self.path_string(nut_depth_coords),
                                                 'id':'nutdepth'})
            nut_bot_path = etree.SubElement(neck_layer,
                                            'path',
                                            **{'style':LINE_STYLE,
                                               'd':self.path_string(nut_bot_coords),
                                               'id':'nutbot'})
            overhang_top_path = etree.SubElement(neck_layer,
                                                 'path',
                                                 **{'style':LINE_STYLE,
                                                    'd':self.path_string(overhang_top_coords),
                                                    'id':'overhangtop'})
            overhang_path = etree.SubElement(neck_layer,
                                             'path',
                                             **{'style':LINE_STYLE,
                                                'd':self.path_string(fb_overhang_coords),
                                                'id':'overhang'})
            overhang_bot_path = etree.SubElement(neck_layer,
                                                 'path',
                                                 **{'style':LINE_STYLE,
                                                    'd':self.path_string(overhang_bot_coords),
                                                    'id':'overhangbot'})
        else:
            nut_top_path = etree.SubElement(hs_layer,
                                            'path',
                                            **{'style':LINE_STYLE,
                                               'd':self.path_string(nut_top_coords),
                                               'id':'nuttop'})
            nut_depth_path = etree.SubElement(hs_layer,
                                              'path',
                                              **{'style':LINE_STYLE,
                                                 'd':self.path_string(nut_depth_coords),
                                                 'id':'nutdepth'})
            nut_bot_path = etree.SubElement(hs_layer,
                                            'path',
                                            **{'style':LINE_STYLE,
                                               'd':self.path_string(nut_bot_coords),
                                               'id':'nuttop'})
        ...
        for i in range(self.n_fret):
            ...
            fret_coords = [(x0-x_dis, y0+y_dis),
                           (x0-x_dis, y0-y_dis)]
            fret_path = etree.SubElement(f_layer,
                                         'path',
                                         **{'style':LINE_STYLE,
                                            'd':self.path_string(fret_coords),
                                            'id':'fret{}'.format(str(fret).zfill(4))})
        ...
        for i in range(self.n_string):
            ...
            string_path = etree.SubElement(str_layer,
                                           'path',
                                           **{'style':LINE_STYLE,
                                              'd':self.path_string(string_coords),
                                              'id':'string{}'.format(str(i+1))})
        ...
        for tuner in range(len(self.l_tuners)):
            ...
            if tuner == len(self.l_tuners)-1:
                ...
                headstock_path = etree.SubElement(hs_layer,
                                                  'path',
                                                  **{'style':LINE_STYLE,
                                                     'd':self.path_string([(headstock_start_x, headstock_start_y), (headstock_end_x, headstock_end_y)]),
                                                     'id':'headstockleft'})
                
            circle = etree.SubElement(hs_layer,
                                      'circle',
                                      **{'style':CIRCLE_STYLE,
                                         'id':'lefttunerpost{}'.format(str(tuner).zfill(4)),
                                         'cx':str(tuner_x),
                                         'cy':str(tuner_y),
            ...
            circle = etree.SubElement(hs_layer,
                                      'circle',
                                      **{'style':CIRCLE_STYLE,
                                         'id':'lefttunerhole{}'.format(str(tuner).zfill(4)),
                                         'cx':str(tuner_x),
                                         'cy':str(tuner_y),
                                         'r':str(self.tuner_hole_r)})
            headstring_path = etree.SubElement(hs_layer,
                                              'path',
                                              **{'style':LINE_STYLE,
                                                 'd':self.path_string([(x0, string_y), (tuner_x, string_y)]),
                                                 'id':'leftheadstockstring{}'.format(str(tuner).zfill(4))})
        ...
        for tuner in range(len(self.r_tuners)):
            ...
            circle = etree.SubElement(hs_layer,
                                      'circle',
                                      **{'style':CIRCLE_STYLE,
                                         'id':'righttunerpost{}'.format(str(tuner_index).zfill(4)),
                                         'cx':str(tuner_x),
                                         'cy':str(tuner_y),
                                         'r':str(self.tuner_post_r)})
            circle = etree.SubElement(hs_layer,
                                      'circle',
                                      **{'style':CIRCLE_STYLE,
                                         'id':'righttunerhole{}'.format(str(tuner_index).zfill(4)),
                                         'cx':str(tuner_x),
                                         'cy':str(tuner_y),
                                         'r':str(self.tuner_hole_r)})
            hstr_path = etree.SubElement(hs_layer,
                                         'path',
                                         **{'style':LINE_STYLE,
                                            'd':self.path_string([(x0, string_y), (tuner_x, string_y)]),
                                            'id':'rightheadstockstring{}'.format(str(tuner_index).zfill(4))})
        ...
        for i in range(self.n_fret):
            ...
            for dot_y in dot_ys:
                dot_id += 1
                dot = etree.SubElement(fm_layer,
                                       'circle',
                                       **{'style':CIRCLE_STYLE,
                                          'id':'fretmarket{}'.format(str(dot_id).zfill(4)),
                                          'cx':str(dot_x),
                                          'cy':str(dot_y),
                                          'r':str(dot_r)})
        ...
        neck_pickup = etree.SubElement(pup_layer,
                                       'path',
                                       **{'style':LINE_STYLE,
                                          'd':self.path_string([(neck_pole_position+x-humbucker_width/4, y0+y) for (x,y) in c_hum_coords], True),
                                          'id':'neckpickup'})
        neck_ring = etree.SubElement(pup_layer,
                                       'path',
                                       **{'style':LINE_STYLE,
                                          'd':self.path_string([(neck_pole_position+x-humbucker_width/4, y0+y) for (x,y) in c_ring_coords], True),
                                          'id':'neckring'})
        neck_mount_hole_bot = etree.SubElement(pup_layer,
                                               'circle',
                                               **{'style':CIRCLE_STYLE,
                                                  'id':'neckmountholetop',
                                                  ...
                                                  'r':'0.75'})
        neck_mount_hole_top = etree.SubElement(pup_layer,
                                               'circle',
                                               **{'style':CIRCLE_STYLE,
                                                  'id':'neckmountholebot',
                                                  'cx':str(neck_pole_position-humbucker_width/4),
    ...
    def save_svg(self):
        with open("./{}".format(self.svg_name), "wb") as f:
